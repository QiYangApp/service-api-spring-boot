package com.myadream.app.qiyang.api.aop;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONException;
import com.myadream.app.qiyang.api.exception.ParamValidException;
...
public class ControllerLoggerAop {
    ...
    @Pointcut("execution(* com.myadream.app.qiyang.api.controller..*(..))")
    ...
    public void doBefore(JoinPoint joinPoint) throws ParamValidException {
        ...
        if (!validResult.isEmpty()) {
            ...
            List<FieldError> errors = validResult.stream().map(constraintViolation -> {
                ...
                FieldError error = new FieldError(paramName, paramName, constraintViolation.getMessage());
                return error;
            }).collect(Collectors.toList());
            // 抛出异常，交给上层处理
            logger.error(HEAD + "参数校验发生错误 ===> {}", errors.toString());
            ParamValidException ex = new ParamValidException(errors);
            logger.error(HEAD + "RESPONSE.ERROR : " + JSON.toJSONString((new DefaultMode<List<FieldError>>()).fail(ex.getFieldErrors())));
