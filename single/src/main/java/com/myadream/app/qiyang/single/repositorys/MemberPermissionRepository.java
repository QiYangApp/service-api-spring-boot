...
public interface MemberPermissionRepository extends JpaRepository<QyMemberEntity, Long> {
    ...
    Collection<QyRouterEntity> getPermissionRoutesByMemberId(Long memberId);
     ...
     * @return Collection<QyRouterEntity>
     */
    @Query(nativeQuery = true, value = "select * from qy_router where id in (select qy_permission_router_relation.router_id from qy_member_role_relation left join qy_role_permission_relation on qy_member_role_relation.role_id = qy_role_permission_relation.role_id left join qy_permission_router_relation on qy_permission_router_relation.permission_group_id = qy_role_permission_relation.permission_group_id where qy_member_role_relation.rold_id = ?1)")
